...
EVN=""
    ...
    if [[ ${EVN} = "" ]]
    then
        echo "enter evn param [ ${EVN_ARRAY[@]} ]"
        exit 0
    fi
...
JAVA_OPTS="-Dspring.cloud.config.profile=$EVN -Dspring.profiles.active=$EVN -Duser.timezone=GMT+8 -server -Xms512m -Xmx1024m -Xloggc:${LOG_DIR}/gc.log"
...
startup(){
    ...
    if [ ${TPID} -ne 0 ]; then
        ...
        getPID
        if [ ${TPID} -ne 0 ]; then
            echo "(PID=$TPID)...[$APPLICATION_MAIN StartUp Success]"
            echo "================================================================================================================"
        else
            echo "[$APPLICATION_MAIN StartUp Failed]"
            echo "================================================================================================================"
        fi
    fi
